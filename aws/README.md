...
- **[Architecture Challenge](architecture-challenge.html)** — a standalone architecture-building game (linked from Home): each challenge hands you a scenario (static site, serverless API, SNS fan-out, an IoT digital twin, broken-diagram fix-it…) and you sketch the AWS architecture the way reference diagrams are drawn — service cards (Lambda, S3, CloudFront, SQS, Step Functions, TwinMaker…) connected by directed data-flow arrows on a canvas. Designs are validated three ways — structural coherence, whether the required flows connect (with per-hop traces), and a best-practice score with explanations (CDN in front of S3, auth on public APIs, no Lambda-to-Lambda chaining…).
