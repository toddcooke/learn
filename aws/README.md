...
- **[The exam shortcut](exam-shortcut.html)** — a standalone static reference (linked from Home) mapping the trigger phrases SAA-C03 stems use to the service each one points at, plus the look-alike service pairs and the qualifier clauses that break a tie between two plausible answers. Hand-maintained plain HTML with no script, like the cheatsheet.
