...
export const FLASHCARDS = [
  ...
  // ---------------------------------------------------------------------
  {
    id: 'osi-model',
    service: 'OSI Model',
    front: 'Name the seven OSI layers in order and one job each performs.',
    back: 'Layer 1 Physical (bits on the wire), Layer 2 Data Link (MAC addressing/framing), Layer 3 Network (IP addressing/routing), Layer 4 Transport (end-to-end delivery), Layer 5 Session (managing a dialogue), Layer 6 Presentation (data formatting), Layer 7 Application (the interface software actually uses). Real-world traffic runs on the leaner TCP/IP model, but OSI persists as the vocabulary for naming which layer a device or protocol operates at.',
  },
  {
    id: 'tcp-udp',
    service: 'TCP vs. UDP',
  ...
  {
    id: 'file-remote-ports',
    service: 'FTP, SSH & RDP Ports',
    front: 'What ports do FTP, SSH, and RDP use?',
    back: "FTP splits into two TCP connections: 21 handles commands, 20 handles the actual file bytes. SSH does everything — command shell, SFTP, SCP — over one encrypted TCP session on port 22. RDP is different in kind: it streams an entire remote Windows desktop, not just text, over TCP port 3389.",
  },
  {
    id: 'name-resolution-ports',
    service: 'DNS & DHCP Ports',
    front: 'What ports do DNS and DHCP use?',
    back: "DNS answers ordinary lookups on UDP port 53, switching to TCP 53 for oversized responses like zone transfers. DHCP splits its two directions across adjacent ports: a server listens for requests on UDP 67, and the client itself gets offers back on UDP 68.",
  },
  {
    id: 'web-ports',
  ...
  {
    id: 'mgmt-signaling-ports',
    service: 'SNMP, LDAP & SIP Ports',
    front: 'What ports do SNMP, LDAP, and SIP use?',
    back: "SNMP splits by direction: a manager polls devices on UDP 161, and devices push unsolicited trap alerts back on UDP 162. LDAP handles directory lookups over TCP 389. SIP signals calls unencrypted on port 5060, or with a TLS-protected signaling channel on port 5061.",
  },
  {
    id: 'traffic-types',
