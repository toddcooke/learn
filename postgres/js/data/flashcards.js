...
export const FLASHCARDS = [
  ...
  },
  {
    ...
    domain: 'Schema Design',
    front: "Why can't a single foreign key express a many-to-many relationship between two tables, and what does the table that resolves it normally use as its primary key?",
    back: "A foreign key records only one reference per row, which is enough for a one-to-many relationship but not for many-to-many, where a row on either side legitimately needs to relate to many rows on the other side no matter which table would hold the key. A junction table resolves this by holding one row per pairing, with a foreign key to each side, and its primary key is normally the composite of those two foreign keys together, since that pair is exactly what keeps a given pairing from being recorded twice.",
  },
  {
