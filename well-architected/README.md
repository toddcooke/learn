...
An unofficial flashcard deck and diagram set covering the AWS Well-Architected Framework — its six pillars, their design principles and best practice areas, the shared vocabulary, and the review process. Not affiliated with or endorsed by AWS.

A companion to [`aws/`](../aws), [`kubernetes/`](../kubernetes), [`postgres/`](../postgres), [`sre/`](../sre), and [`networking/`](../networking).

## Reading it

No install step, no server. Open [`flashcards.md`](flashcards.md) directly in any markdown viewer (GitHub, VS Code, Obsidian, etc.) — every answer is hidden in a collapsed `<details>` block so you can self-test. [`diagrams.md`](diagrams.md) is read the same way; its diagrams are Mermaid, which Obsidian and GitHub render natively (VS Code needs a Mermaid preview extension). [`cheatsheet.html`](cheatsheet.html) is a standalone HTML page — open it by double-click, no server needed.
...
This module has no `study/` directory and no `questions.md`, unlike its five siblings — just the flashcard deck, a set of diagrams, and a cheatsheet.
...
- **Diagrams** — [`diagrams.md`](diagrams.md), 10 Mermaid diagrams for the structure rather than the recall: the framework at a glance, the vocabulary nesting, the review process, the trade-offs between pillars, and one map per pillar.
